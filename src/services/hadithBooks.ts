/**
 * Hadith collections proxy. The mobile app calls our API only; we fetch from
 * Fawaz Ahmed's Hadith API (fawazahmed0/hadith-api, jsDelivr) server-side and
 * cache it. Mirrors the source's structure: Book → Edition (language) →
 * Sections (chapters) → Hadiths.
 */
...
// Editions are immutable, so cache generously. `useClones:false` avoids copying
// the large full-book objects on every read.
...
export async function getCollections(): Promise<CollectionDto[]> {
  ...
  const list: CollectionDto[] = Object.entries<any>(data).map(([key, v]) => ({
    key,
    name: v.name,
    editions: (v.collection ?? []).map((e: any) => ({
      name: e.name,
      language: e.language,
    })),
  }));
...
export async function getSections(edition: string): Promise<SectionDto[]> {
  ...
  // Each hadith's reference.book is its section number — group by it to get the
  // hadith range per section (robust across editions/metadata-key variations).
  ...
  for (const h of full.hadiths ?? []) {
    ...
    if (!g) {
      groups.set(b, { first: h.hadithnumber, last: h.hadithnumber });
    } else {
  ...
  for (const [b, g] of groups) {
    ...
    if (!name) continue; // skip the unnamed catch-all (e.g. book 0)
